...
with open('../data/ILSVRC2012/wnid_to_label_org.pickle', 'rb') as f:
    map_org = pickle.load(f)

with open('../data/ILSVRC2012/wnid_to_label.pickle', 'rb') as f:
    map_new = pickle.load(f)

rev_map_new = {v:k for k,v in map_new.items()}

def convert_to_imagenet_labs(lab_woof):
    lab_id = rev_map_new[lab_woof]
    return map_org[lab_id]
...
def test_accuracy(model, test_dl, device, topk=(1, )):
    ...
    for j, (x_test, target) in enumerate(tqdm(test_dl)):
        ...
        target = target.apply_(convert_to_imagenet_labs)
