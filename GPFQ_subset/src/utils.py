...
def test_accuracy(model, test_dl, device, topk=(1, )):
    ...
    for j, x_test_and_target in enumerate(tqdm(test_dl)):
        for i in range(10):
            x_test_and_target[-1][x_test_and_target[-1]==i] = mapping[i]
        
        target = x_test_and_target[-1]
        x_test = x_test_and_target[0]
